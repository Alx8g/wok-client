...
/**
 * Clean metrics whose score is (0.7 x averageFps + 80) up to rounding, so trial scores can be
 * targeted directly: consistency is 1, no long frames, worst equals p95.
 */
...
/** Runs the standard A|B screen stage plus the ABAB repeat stage with targeted per-trial scores. */
function runStageTwo(state: CalibrationState, { explicitScores, fallbackRenderer = 'Unknown renderer', fallbackScores }: StageTwoScores): CalibrationState {
	...
	assert.ok(next.plan.some(slot => slot.stage === 'repeat'), 'expected close screen results to append the repeat stage');
...
test('the persisted plan is followed deterministically across simulated relaunches', () => {
	...
	assert.deepEqual(state.plan.map(slot => `${slot.candidateId}#${slot.launchGroup}`), ['d3d11on12:uncapped#1', 'default:uncapped#2']);

	// Relaunch: the state round-trips through persistence and resumes the identical plan.
...
test('close screen results append an ABAB repeat block with shared launch groups', () => {
	...
	assert.deepEqual(state.plan.map(slot => `${slot.candidateId}:${slot.stage}:${slot.launchGroup}`), [
		'd3d11on12:uncapped:screen:1',
		'default:uncapped:screen:2',
		'd3d11on12:uncapped:repeat:3',
		'd3d11on12:uncapped:repeat:3',
		'default:uncapped:repeat:4',
		'default:uncapped:repeat:4'
	]);
...
test('verified backend evidence breaks ties between explicit candidates', () => {
	...
	assert.deepEqual(candidates.map(candidate => candidate.id), ['d3d11:uncapped', 'd3d11on12:uncapped']);
...
test('the GPU-time tie-breaker fires only when both candidates have measured GPU timing', () => {
	...
	const explicitMetrics = (score: number, gpuTimeP95Ms: number | undefined, renderer: string): CalibrationMetrics => metricsForScore(score, {
		webglRenderer: renderer,
		...(gpuTimeP95Ms !== undefined
			? { gpuTimeP95Ms, gpuTimingStatus: 'measured' as const }
			: { gpuTimingStatus: 'unsupported' as const })
	});
