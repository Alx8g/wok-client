...
import { MATCHMAKER_GAMEMODES, MATCHMAKER_MAP_SCOPES, MATCHMAKER_OFFICIAL_MAPS, MATCHMAKER_REGIONS } from './matchmaker-data.ts';
import { SettingsRefreshTracker, type SettingsRefreshRequirement } from './settings-refresh.ts';
import { CUSTOM_CLAN_PREFERENCE_KEY, isCustomIdentityPreferenceKey, REAL_CLAN_PREFERENCE_KEY, sanitizeCustomClan, sanitizeCustomName } from './custom-identity.ts';
...
ipcRenderer.on('m_userPrefs_for_settingsUI', (_event, received_paths: IPaths, received_userPrefs: UserPrefs, received_displays: DisplayOption[]) => {
	const displayOptions = Array.isArray(received_displays) && received_displays.length > 0 ? received_displays : [{ value: DISPLAY_PREFERENCE_AUTO, label: 'Automatic (primary display)' }];
...
function applyThemeSelection(value: string) {
	void applyTheme(value, paths.cssPath).catch((error) => {
		strippedConsole.error(`Failed to apply the theme ${value}`, error);
	});
}
const settingsDesc: SettingsDesc = {
	competitiveMode: {
		...
		type: 'bool',
		desc: 'Measures your PC and applies the fastest graphics profile, plus lower in-game visuals for more FPS. Your Krunker settings are restored if you turn it off.',
		safety: 0,
		cat: 0
	},
	...
	fullscreen: {
		title: 'Window Mode',
		type: 'sel',
		desc: 'Fullscreen gives the smoothest frames.',
		safety: 0,
		cat: 0,
		opts: ['windowed', 'maximized', 'fullscreen', ...(process.platform !== 'win32' ? ['borderless'] : [])]
	},
	...
	customName: {
		title: 'Custom Name',
		type: 'text',
		desc: 'Shows this name instead of yours in chat, the scoreboard, the kill feed and the menu. Only on your screen; Krunker still gets your real one.',
		placeholder: 'Leave empty for your real name',
		safety: 0,
		cat: 1,
		instant: true
	},
	customClan: {
		title: 'Custom Clan',
		type: 'text',
		desc: 'Shows this clan tag instead of yours, everywhere the game prints it. Only on your screen.',
		placeholder: 'Leave empty for your real clan',
		safety: 0,
		cat: 1,
		instant: true
	},
	realName: {
		...
		type: 'text',
		desc: 'Only needed if the custom name is not being applied: the exact Krunker name to replace. The client normally detects this by itself.',
	...
	realClan: {
		title: 'Your Real Clan',
		type: 'text',
		desc: 'Only needed if the custom clan tag is not being applied: your real tag, without brackets.',
		placeholder: 'Detected automatically',
		safety: 0,
		cat: 1,
		instant: true
	...
	motionBlurQuality: {
		...
		type: 'sel',
		desc: 'Native preserves full sharpness. Lower resolutions reduce GPU work but soften the image while turning.',
		safety: 0,
		cat: 2,
		instant: true,
		opts: ['native', 'balanced', 'performance'],
		optLabels: ['Native (100%)', 'Balanced (75%)', 'Performance (50%)']
	},
	theme: themeOption,
	...
	matchmaker_maps: {
		title: 'Maps',
		type: 'multisel',
		desc: 'Narrows Official or Selected scope to these maps. Leave empty with Official scope to allow every official map.',
		safety: 0,
		cat: 3,
		opts: MATCHMAKER_OFFICIAL_MAPS,
		cols: 4,
		instant: true
	},
	...
	safeFlags_highPerformanceGpu: {
		title: 'Prefer High-Performance GPU',
		type: 'bool',
		desc: 'On laptops with two GPUs, uses the fast one. If diagnostics still show integrated graphics, set it for WOK in your OS graphics settings too.',
		safety: 1,
		cat: 4
	},
...
const safetyDesc = ['This setting is safe/standard', 'Proceed with caution', 'This setting is not recommended', 'This setting is experimental', 'This setting is experimental and unstable. Use at your own risk.'];
...
function settingValuesEqual(setting: UserPrefs[keyof UserPrefs], cachedSetting: UserPrefs[keyof UserPrefs]): boolean {
...
function updateRefreshNeededForKey(key: string) {
	refreshNeeded = settingsRefreshTracker.update(key, !settingValuesEqual(userPrefs[key], userPrefsCache[key]), refreshRequirementForKey(key));
}
...
class SettingElem {
	...
	update(elem: HTMLElement, callback: Callbacks, event?: InputEvent) {
		...
		if (this.props.type === 'num') {
			...
			if (Object.hasOwn(this.props, 'min') && dirtyValue < this.props.min) {
				dirtyValue = this.props.min;
				updateUI();
			}
			if (Object.hasOwn(this.props, 'max') && dirtyValue > this.props.max) {
	...
	get elem() {
		...
		(wrapper as unknown as { onchange?: unknown; oninput?: unknown })[this.updateMethod as 'onchange' | 'oninput'] = (event: InputEvent) => {
...
let settingElementPairs: {
	[key: string]: SettingElem;
} = {};
...
function toggleSettingsCategory(header: Element) {
	...
	iconElement.textContent = iconElement.textContent === 'keyboard_arrow_down' ? 'keyboard_arrow_right' : 'keyboard_arrow_down';
}
...
export function renderSettings() {
	...
	Array.from(document.querySelectorAll('.setHed'))
		.filter((element) => element.innerHTML === 'No settings found')
		.forEach((element) => element.remove());
	crankshaftSettingsHolder.remove();
	crankshaftSettingsHolder.replaceChildren();
	settingElementPairs = {};
	const settings = transformMarrySettings(userPrefs, settingsDesc, 'normal').filter((setting) => settingSearchFilter(setting, filter));
