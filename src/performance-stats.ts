export const PERFORMANCE_WINDOW_MS = 10000;
export const CURRENT_FPS_WINDOW_MS = 1000;
...
export class RollingPerformanceStats {
	...
	snapshot(now: number): PerformanceSnapshot {
		...
		const windowSeconds = this.windowSampleCount > 0 ? Math.min(PERFORMANCE_WINDOW_MS, Math.max(0, now - earliestSampleTime)) / 1000 : 0;
		return {
