...
export const GRAPHICS_QUARANTINE_BASE_MS = 5 * 60 * 1000;
export const GRAPHICS_QUARANTINE_MAX_MS = 24 * 60 * 60 * 1000;
...
function isGraphicsLaunchOutcome(value: unknown): value is GraphicsLaunchOutcome {
	return value === 'idle' || value === 'pending' || value === 'completed' || value === 'clean-interruption' || value === 'unknown-interruption' || value === 'gpu-failure';
}
...
export function normalizeGraphicsBackend(value: unknown): GraphicsBackend {
	return typeof value === 'string' && GRAPHICS_BACKENDS.includes(value as GraphicsBackend) ? (value as GraphicsBackend) : 'auto';
}
...
export function assessIntegratedGpuUsage(devices: GraphicsDevice[], webglRenderer = ''): IntegratedGpuAssessment {
	...
	const integratedActive = activeDevices.length > 0 ? activeDevices.every((device) => device.vendorId === INTEL_VENDOR_ID) : rendererLooksIntel;
...
export function recommendGraphicsBackend(platform: NodeJS.Platform, devices: GraphicsDevice[]): Pick<GraphicsProfileState, 'recommendedBackend' | 'recommendationReason'> {
	...
	const hardwareDevices = devices.filter((device) => device.vendorId > 0 && device.vendorId !== 0xffff && device.vendorId !== MICROSOFT_SOFTWARE_VENDOR_ID);
...
function activeQuarantinedBackends(state: GraphicsProfileState, now: number): AppliedGraphicsBackend[] {
	...
	for (const backend of state.blockedBackends) {
		if (backend !== 'default' && !failuresByBackend.has(backend) && state.updatedAt + GRAPHICS_QUARANTINE_BASE_MS > now) active.add(backend);
	}
	return GRAPHICS_BACKENDS.filter((backend): backend is AppliedGraphicsBackend => backend !== 'auto' && active.has(backend));
}
export function isGraphicsBackendQuarantined(state: GraphicsProfileState, backend: AppliedGraphicsBackend, now: number = Date.now()): boolean {
	return activeQuarantinedBackends(state, now).includes(backend);
}
export function releaseExpiredGraphicsQuarantines(state: GraphicsProfileState, now: number = Date.now()): GraphicsProfileState {
...
export function createGraphicsProfileState(platform: NodeJS.Platform = process.platform, now: number = Date.now()): GraphicsProfileState {
...
function parseBackendFailures(value: unknown): GraphicsBackendFailure[] {
	...
	for (const entry of value) {
		...
		const failureCount = Number.isInteger(entry.failureCount) && Number(entry.failureCount) > 0 ? Number(entry.failureCount) : 1;
...
export function parseGraphicsProfileState(value: unknown, now: number = Date.now()): GraphicsProfileState | undefined {
	...
	const persistedBlockedBackends = Array.isArray(value.blockedBackends) ? value.blockedBackends.map(normalizeGraphicsBackend).filter((backend): backend is AppliedGraphicsBackend => backend !== 'auto') : [];
	...
	const gpuFailureCount = Number.isInteger(value.gpuFailureCount) && Number(value.gpuFailureCount) >= 0 ? Number(value.gpuFailureCount) : 0;
...
export function updateGraphicsDetection(state: GraphicsProfileState, platform: NodeJS.Platform, devices: GraphicsDevice[], now: number = Date.now()): GraphicsProfileState {
...
export function updateGraphicsDriverIdentity(state: GraphicsProfileState, driverFingerprint: string, now: number = Date.now()): GraphicsProfileState {
...
export function clearKeptGraphicsBackend(state: GraphicsProfileState, now: number = Date.now()): GraphicsProfileState {
...
function recordGpuFailure(state: GraphicsProfileState, backend: AppliedGraphicsBackend, reason: string, quarantine: boolean, now: number): GraphicsProfileState {
	...
	const backendFailures = [...state.backendFailures.filter((existing) => existing.backend !== backend), failure];
...
export function recordGraphicsGpuFailure(state: GraphicsProfileState, backend: AppliedGraphicsBackend, reason: string, now: number = Date.now()): GraphicsProfileState {
	return recordGpuFailure(state, backend, reason, true, now);
}
export function recordManualGraphicsGpuFailure(state: GraphicsProfileState, backend: AppliedGraphicsBackend, reason: string, now: number = Date.now()): GraphicsProfileState {
	return recordGpuFailure(state, backend, reason, false, now);
}
export function describeManualBackendFailures(state: GraphicsProfileState, selection: Pick<GraphicsSelection, 'backend' | 'source'>): string | undefined {
...
export function recordGraphicsFailure(state: GraphicsProfileState, backend: AppliedGraphicsBackend, reason: string, now: number = Date.now()): GraphicsProfileState {
	return recordGraphicsGpuFailure(state, backend, reason, now);
}
export function recordGraphicsLaunchInterruption(state: GraphicsProfileState, kind: GraphicsLaunchInterruptionKind, reason: string, now: number = Date.now()): GraphicsProfileState {
...
export function recordCleanGraphicsLaunchInterruption(state: GraphicsProfileState, reason = 'The graphics launch ended during a clean application shutdown.', now: number = Date.now()): GraphicsProfileState {
	return recordGraphicsLaunchInterruption(state, 'clean', reason, now);
}
export function recordUnknownGraphicsLaunchInterruption(state: GraphicsProfileState, reason = 'The graphics launch ended without a success or GPU-failure signal.', now: number = Date.now()): GraphicsProfileState {
	return recordGraphicsLaunchInterruption(state, 'unknown', reason, now);
}
export function recoverInterruptedGraphicsLaunch(state: GraphicsProfileState, reason?: string, now: number = Date.now()): GraphicsProfileState {
	if (!state.launchPending) return releaseExpiredGraphicsQuarantines(state, now);
	const selectionLabel = state.lastSelectionSource === 'calibration' ? 'calibrated' : state.lastSelectionSource;
	return recordUnknownGraphicsLaunchInterruption(state, reason ?? `The previous ${selectionLabel} ${state.lastAppliedBackend} launch ended before a success or GPU-failure signal was recorded.`, now);
}
export function recoverInterruptedAutoLaunch(state: GraphicsProfileState, now: number = Date.now()): GraphicsProfileState {
...
export function selectGraphicsBackend(preferenceValue: unknown, state: GraphicsProfileState, platform: NodeJS.Platform = process.platform, now: number = Date.now()): GraphicsSelection {
...
export function beginGraphicsLaunch(state: GraphicsProfileState, selection: GraphicsSelection, now: number = Date.now()): GraphicsProfileState {
	return {
		...state,
...
export function completeGraphicsLaunch(state: GraphicsProfileState, now: number = Date.now()): GraphicsProfileState {
