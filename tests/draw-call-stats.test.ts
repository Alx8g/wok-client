...
import {
	...
	summarizeDrawCallCensus,
	type DrawCallCensusReport,
	type DrawCallCensusTarget
} from '../src/draw-call-stats.ts';

function createFakeGlPrototype(): DrawCallCensusTarget & { calls: string[] } {
	const target = { calls: [] as string[] } as DrawCallCensusTarget & { calls: string[] };
	for (const name of DRAW_METHOD_NAMES) target[name] = function () { (this as typeof target).calls.push(name); return name; };
	target.bindTexture = function () { (this as typeof target).calls.push('bindTexture'); };
	target.useProgram = function () { (this as typeof target).calls.push('useProgram'); };
	return target;
}
...
test('summarizes per-frame counts as medians and p95s', () => {
	...
	assert.deepEqual(summarizeDrawCallCensus(samples), {
		...
		// House percentile convention (same as calibration-benchmark): index floor((n-1)*ratio),
		// so a four-sample p95 lands on the third value rather than the outlier.
		p95Draws: 30,
...
test('counts real calls per frame, skips warmup and idle frames, then restores the prototype', () => {
	...
	drive(999, 999, 999); // warmup frame: excluded entirely
	drive(0, 5, 5);       // idle frame (no draws): excluded so menus cannot dilute the census
	drive(40, 8, 3);
	drive(60, 12, 5);
	...
	// The census must leave no wrappers behind once it has reported.
...
test('only gameplay frames are censused, so a menu cannot fill the window', () => {
	...
	// Menu frames render constantly at high frame rates; none of them may be counted.
	for (let index = 0; index < 50; index++) drive(60);
	assert.equal(report, undefined, 'menu frames must not fill the census window');

	active = true;
	drive(300); // warmup, now that gameplay is live
	drive(280);
	drive(320);
