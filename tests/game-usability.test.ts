...
import { describeGameUsability, formatGameUsabilitySnapshot, GameUsabilitySignal, isGameUsable, observeGameUsability } from '../src/game-usability.ts';
class FakeElement {
	...
	public querySelector(selector: string): FakeElement | null {
		return selector === '.lds-ring' && this.hasLoadingSpinner ? new FakeElement() : null;
	}
}
class FakeDocument {
...
function createObservationHarness(options: { createMutationObserver?: (callback: MutationCallback) => never; onUsable?: () => void } = {}) {
...
test('readiness ignores Krunker loading instructions until the overlay clears', () => {
	...
	assert.equal(harness.document.listenerCount('pointerlockchange'), 1);
	...
	assert.equal(harness.document.listenerCount('pointerlockchange'), 0);
	harness.triggerMutation();
...
test('the readiness snapshot describes why a load is stuck without ever throwing', () => {
	...
	assert.equal(formatGameUsabilitySnapshot(stillLoading), 'readyState=interactive pointerLocked=false instructions=present instructionsText=7 spinner=true loadingBg=visible');
	...
	assert.match(formatGameUsabilitySnapshot(foreignPage), /instructions=missing .*loadingBg=missing/u);
	const hostile = {
	...
	const errors: unknown[] = [];
	const snapshot = describeGameUsability(hostile as unknown as Document, (error) => {
		errors.push(error);
	});
...
test('pointer lock independently reports usability and cancellation detaches observers', () => {
	...
	assert.equal(cancelledHarness.document.listenerCount('pointerlockchange'), 0);
});
