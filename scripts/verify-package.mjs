...
function resourcesPathFor(buildPath, platform) {
	...
	assert.ok(
		appBundle,
		`Could not find the packaged macOS app in ${buildPath}.`
	);
	return join(
		buildPath,
		appBundle.name,
...
function expectedBundleEntries(repositoryRoot) {
	const metafile = JSON.parse(readFileSync(join(
		repositoryRoot,
		'bundle',
		'metafile.json'
	), 'utf8'));
	return new Set(Object.keys(metafile.outputs)
...
function assertExactEntries(actual, expected, label) {
	const missing = [...expected].filter(entry =>
		!actual.has(entry));
	const unexpected = [...actual].filter(entry =>
		!expected.has(entry));
	assert.deepEqual(
		missing,
		[],
	...
	assert.deepEqual(
		unexpected,
		[],
		`${label} contain unexpected files: ${unexpected.join(', ')}`
	);
}
...
export function verifyPackagedApplication({
	...
	const resourcesPath = resourcesPathFor(
		buildPath,
		platform
	);
	...
	assert.ok(
		existsSync(asarPath),
		`Packaged application is missing ${asarPath}.`
	);
	...
	for (const prefix of FORBIDDEN_PREFIXES) {
		assert.ok(
			...
			`Packaged ASAR contains forbidden ${prefix} content.`
		);
	}
	assert.ok(
		!entries.some(entry =>
			...
			|| entry.endsWith('.ts')
			|| entry.endsWith('.tsx')),
		'Packaged ASAR contains source or source-map files.'
	);
	assert.ok(
		!entrySet.has('bundle/metafile.json'),
		'Packaged ASAR must not contain the esbuild metafile.'
	);

	const packagedAssets = new Set(entries.filter(entry =>
	...
	const packagedBundle = new Set(entries.filter(entry =>
		entry.startsWith('bundle/')
		&& !entry.endsWith('/')));
	assertExactEntries(
		packagedBundle,
		expectedBundleEntries(repositoryRoot),
		'Packaged bundle outputs'
	);
	...
	assert.equal(
		packageJson.main,
		'./bundle/main.mjs',
		'Packaged package.json must start the bundled main entry.'
	);
	assert.equal(
		packageJson.private,
		true,
		'Packaged package.json must retain private=true.'
	);

	for (const [notice, source] of REQUIRED_EXTERNAL_NOTICES) {
		const packagedNoticePath = join(resourcesPath, notice);
		assert.ok(
			existsSync(packagedNoticePath),
			`Packaged resources are missing ${notice}.`
		);
		assert.deepEqual(
			readFileSync(packagedNoticePath),
			readFileSync(join(repositoryRoot, source)),
			`Packaged ${notice} does not match repository ${source}.`
		);
	}
...
if (process.argv[1] === import.meta.filename) {
	const [buildPath, platform = process.platform] =
		process.argv.slice(2);
	assert.ok(
		buildPath,
		'Usage: node scripts/verify-package.mjs <build-path> [platform]'
	);
	...
	console.log(
		`verify-package: ok (${result.entryCount} ASAR entries; `
			+ `${result.bundleOutputCount} bundle outputs; `
