import { isCustomIdentityPreferenceKey, parseCustomIdentityPreference } from './custom-identity.ts';
import { parseThemePreference } from './themes.ts';
import { isDisplayPreference } from './display-selection.ts';
const OBSOLETE_PREFERENCE_KEYS = new Set(['saveMatchResultJSONButton', 'cssSwapper', 'experimentalFlags_increaseLimits', 'inProcessGPU', 'loadingSplashTitleCardBackgroundColor', 'motionBlurShutterAngle', 'userscripts']);
...
const KEYBIND_PREFERENCE_KEYS = new Set(['matchmakerAcceptKey', 'matchmakerCancelKey', 'matchmakerKey']);
const STRING_ARRAY_PREFERENCE_KEYS = new Set(['matchmaker_gamemodes', 'matchmaker_maps', 'matchmaker_regions']);
...
function parseKrunkerOverrideUrl(value: unknown): string | undefined {
	...
	try {
		...
		if (url.protocol !== 'https:' || (url.hostname !== 'krunker.io' && !url.hostname.endsWith('.krunker.io')) || url.username || url.password) return undefined;
		return url.toString();
	} catch (_error) {
		return undefined;
	}
...
function parsePreferenceValue(key: string, value: unknown): UserPrefValue | undefined {
	...
	if (numberRange) {
		return typeof value === 'number' && Number.isInteger(value) && value >= numberRange[0] && value <= numberRange[1] ? value : undefined;
	}
