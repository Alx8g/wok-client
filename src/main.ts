...
import { createStartupProfile, estimateProcessStartWallClockMs, INTRO_VARIANTS, parseStartupProfile, recordStartupSample, selectIntroVariant, startupReadyMs, type StartupProfile } from './startup-profile.ts';
import {
...
import { MATCHMAKER_PING_LIST_MAX_RESPONSE_BYTES, MATCHMAKER_PING_LIST_URL, MatchmakerRegionLatencyService, type MatchmakerPingTarget } from './matchmaker-latency.ts';
...
import { buildUserThemeTemplate, migrateThemePreference, THEME_LAYER_ASSETS, THEME_NONE, THEME_TEMPLATE_FILE, THEME_TEMPLATE_STARTER_ID, themeAssetName } from './themes.ts';
import { containsObsoletePreferences, parseUserPreferencePatch, shouldMigrateMatchmakerMapScope } from './user-preferences.ts';
import { resolveGameplayWindowGeometry } from './window-geometry.ts';
import { buildDisplayOptions, DISPLAY_PREFERENCE_AUTO, selectGameplayDisplay, type DisplayOption } from './display-selection.ts';
import {
...
import { RUNTIME_PROFILE_DURATION_MS, RUNTIME_PROFILE_SAMPLE_INTERVAL_US, RuntimeProfiler, runtimeProfileRequested, type RuntimeProfilePaths } from './runtime-profiler.ts';
...
const traceDelayMs = Number.isFinite(parsedTraceDelayMs) && parsedTraceDelayMs >= 0 ? parsedTraceDelayMs : 3000;
...
async function recordDiagnosticTrace(): Promise<void> {
	...
	try {
		...
		await new Promise((resolve) => {
			setTimeout(resolve, traceDurationMs);
		});
...
function parseKrunkerUrl(value: string, allowEditor = false): URL | undefined {
	...
	try {
		...
		if (url.protocol !== 'https:' || !isKrunkerHostname(url.hostname) || (!allowEditor && url.hostname === 'editor.krunker.io') || url.username || url.password) return undefined;
		return url;
	} catch (_error) {
		return undefined;
	}
...
function parseClientUrl(value: string): string | undefined {
	...
	try {
		const url = new URL(value);
		return clientProtocols.some((clientProtocol) => url.protocol === `${clientProtocol}:`) ? url.toString() : undefined;
	} catch (_error) {
		return undefined;
	}
}
...
if (!gotTheLock) {
	...
	const protocolClientArguments = process.defaultApp && process.argv.length >= 2 ? [pathResolve(process.argv[1])] : undefined;
	for (const clientProtocol of clientProtocols) {
		const isProtocolClient = protocolClientArguments ? app.isDefaultProtocolClient(clientProtocol, process.execPath, protocolClientArguments) : app.isDefaultProtocolClient(clientProtocol);
	...
	try {
		...
		if (migration.foundSources.length > 0) {
			console.log(
				`Migrated ${migration.copiedFiles} startup-critical legacy configuration files from ${migration.foundSources.join(', ')}; preserved ${migration.skippedConflicts} existing WOK Client files. Remaining files migrate in the background after the game window opens.`
			);
		}
	} catch (error) {
	...
	function loadGraphicsProfile(): GraphicsProfileState {
		...
		try {
			return parseGraphicsProfileState(JSON.parse(readFileSync(graphicsProfilePath, 'utf-8'))) ?? createGraphicsProfileState();
		} catch (error) {
	...
	function calibrationDriverFingerprint(gpuInfo: unknown): string {
		...
		return createHash('sha256')
			.update(driverValues.join('|') || 'unknown-driver')
			.digest('hex');
	}
	function normalizeBenchmarkMetrics(value: unknown): CalibrationMetrics {
		...
		const lowConfidenceReasons = Array.isArray(metrics.lowConfidenceReasons)
			? [
					...new Set(
						metrics.lowConfidenceReasons.filter((reason): reason is CalibrationLowConfidenceReason => typeof reason === 'string' && CALIBRATION_LOW_CONFIDENCE_REASONS.includes(reason as CalibrationLowConfidenceReason))
					)
				]
			: [];
		const rejectionReasons = Array.isArray(metrics.rejectionReasons)
			? [
					...new Set(
						metrics.rejectionReasons.filter((reason): reason is CalibrationTrialRejectionReason => typeof reason === 'string' && CALIBRATION_TRIAL_REJECTION_REASONS.includes(reason as CalibrationTrialRejectionReason))
					)
				]
			: [];
		const contaminationFlags = Array.isArray(metrics.contaminationFlags) ? [...new Set(metrics.contaminationFlags.filter((flag): flag is string => typeof flag === 'string' && flag.length <= 64))] : [];
		const gpuTimingStatus: CalibrationGpuTimingStatus = metrics.gpuTimingStatus === 'measured' || metrics.gpuTimingStatus === 'unreliable' ? metrics.gpuTimingStatus : 'unsupported';
		const rawEnvironment = metrics.environment && typeof metrics.environment === 'object' && !Array.isArray(metrics.environment) ? (metrics.environment as Record<string, unknown>) : undefined;
		...
		return {
			...
			webglRenderer: typeof metrics.webglRenderer === 'string' ? metrics.webglRenderer.slice(0, 1024) : '',
	...
	let graphicsProfileState = releaseExpiredGraphicsQuarantines(recoverInterruptedGraphicsLaunch(loadGraphicsProfile()));
	...
	if (
		...
		calibrationState = recordCalibrationResult(calibrationState, queuedCalibrationCandidate, failedCalibrationMetrics(), graphicsProfileState.lastFailureReason);
	...
	const activeCalibrationSelection = userPrefs.competitiveMode && calibrationState?.status === 'complete' ? calibrationState.activeSelection : undefined;
	...
	const graphicsSelection: GraphicsSelection = process.argv.includes('--safe-graphics')
		...
		: calibratedCandidate && !isGraphicsBackendQuarantined(graphicsProfileState, calibratedCandidate.backend)
			? {
					...
					preference: 'auto',
					reason: queuedCalibrationCandidate ? `Running calibration profile ${calibratedCandidate.id}.` : `Using calibrated Competitive mode profile ${calibratedCandidate.id}.`,
					source: 'calibration'
				}
			: selectGraphicsBackend(userPrefs.graphicsBackend, graphicsProfileState);
	const effectiveFramePolicy: FramePolicy = queuedCalibrationCandidate?.framePolicy ?? activeCalibrationSelection?.candidate.framePolicy ?? (userPrefs.fpsUncap ? 'uncapped' : 'capped');
...
`
	...
	function ensureCssStorage() {
		...
		try {
			const layers = THEME_LAYER_ASSETS.map((asset) => readFileSync(pathJoin($assets, asset), { encoding: 'utf-8' })).join('\n');
			writeFileSync(themeTemplatePath, buildUserThemeTemplate(layers, readFileSync(pathJoin($assets, themeAssetName(THEME_TEMPLATE_STARTER_ID)), { encoding: 'utf-8' })), { encoding: 'utf-8' });
		} catch (error) {
	...
	const settingsBaselinePlan = settingsBaselineMarkerUnreadable ? { patch: {} } : planSettingsBaseline(settingsBaselineMarker, indexedUserPrefs);
	...
	if (typeof userPrefs.fullscreen === 'boolean') {
		modifiedSettings = true;
		if (userPrefs.fullscreen === true) userPrefs.fullscreen = 'fullscreen';
		else userPrefs.fullscreen = 'windowed';
	}
	if (typeof userPrefs.hideAds === 'boolean') {
		modifiedSettings = true;
		if (userPrefs.hideAds === true) userPrefs.hideAds = 'hide';
		else userPrefs.hideAds = 'off';
	}
	...
	function probeMatchmakerPingTarget(target: MatchmakerPingTarget, signal: AbortSignal): Promise<number | undefined> {
		return new Promise((resolve) => {
			...
			socket.once('connect', () => {
				...
				finish(Number(elapsedNs) / 1000000);
			});
	...
	ipcMain.on('wok_loading_diag', (event, line: unknown) => {
		...
		if (typeof line !== 'string' || line.length === 0 || line.length > 2000) return;
...
`,
	...
	if (process.env.WOK_FPS_LOG) {
		ipcMain.on('wok_fps_log', (event, sample: unknown) => {
			...
			console.log(
				`[wok-fps] backend=${graphicsSelection.backend} avg=${numeric('averageFps').toFixed(1)} 1%low=${numeric('onePercentLowFps').toFixed(1)} p95=${numeric('p95FrameTimeMs').toFixed(2)}ms worst=${numeric('worstFrameTimeMs').toFixed(2)}ms samples=${numeric('sampleCount')} window=${numeric('windowSeconds').toFixed(1)}s`
			);
		});
	}
	if (process.env.WOK_DRAW_STATS) {
		ipcMain.on('wok_draw_stats', (event, report: unknown) => {
			...
			const numeric = (key: string) => (typeof values[key] === 'number' && Number.isFinite(values[key]) ? Number(values[key]) : -1);
			console.log(
				`[wok-draw-census] frames=${numeric('frames')} draws=${numeric('medianDraws')} (p95 ${numeric('p95Draws')}, max ${numeric('maxDraws')}) textureBinds=${numeric('medianTextureBinds')} (p95 ${numeric('p95TextureBinds')}) programSwitches=${numeric('medianProgramSwitches')} (p95 ${numeric('p95ProgramSwitches')})`
			);
		});
	}
	if (perfMarksEnabled) {
	...
	app.on('gpu-info-update', () => {
		gpuFeatureStatus = Object.fromEntries(Object.entries(app.getGPUFeatureStatus()).filter((entry): entry is [string, string] => typeof entry[1] === 'string'));
	});
	...
	function parseCompetitiveModeBackup(value: unknown): CompetitiveModeBackup | undefined {
		...
		const settings = Object.fromEntries(
			Object.entries(backup.settings as Record<string, unknown>).filter(
				([key, settingValue]) => competitiveGameSettingKeys.has(key) && (typeof settingValue === 'boolean' || typeof settingValue === 'number' || typeof settingValue === 'string')
			)
	...
	function enqueueSettingsWrite() {
		...
		settingsWriteQueue = settingsWriteQueue
			...
			.then(() => {
				persistedSettingsRevision = Math.max(persistedSettingsRevision, revision);
			})
			.catch((error) => {
				console.error('Failed to save WOK Client settings', error);
			});
	}
	...
	app.on('child-process-gone', (_event, details) => {
		if (appQuitting || details.type !== 'GPU' || !graphicsFailureReasons.has(details.reason) || graphicsSelection.source === 'recovery') return;
		const reason = `GPU process ${details.reason} with exit code ${details.exitCode}.`;
		const failedState =
			graphicsSelection.source === 'manual' ? recordManualGraphicsGpuFailure(graphicsProfileState, graphicsSelection.backend, reason) : recordGraphicsGpuFailure(graphicsProfileState, graphicsSelection.backend, reason);
	...
	function observeGraphicsLaunchRenderer(window: BrowserWindow, onRendererGone?: () => void) {
		window.webContents.on('render-process-gone', (_event, details) => {
			...
			graphicsProfileState = recordUnknownGraphicsLaunchInterruption(graphicsProfileState, `Renderer process ${details.reason} with exit code ${details.exitCode}; no GPU-process failure was observed.`);
			persistGraphicsProfile();
		});
	}
	...
	function persistCalibrationRetryState(next: CalibrationState): void {
		if (!writeCalibrationStateSync(next)) {
			throw new Error('Calibration retry state could not be persisted; refusing to launch another attempt.');
		}
		calibrationState = next;
	}
	...
	async function offerCalibrationForCompetitiveEnable(): Promise<void> {
		...
		try {
			const result = await dialog.showMessageBox(mainWindow, {
				...
				defaultId: 0,
				detail:
					'Calibration takes under a minute and the app restarts a few times while renderer profiles are measured. Declining keeps Competitive mode on with the recommended settings; you can calibrate any time from Settings.',
	...
	async function maybeShowStaleCalibrationPrompt(): Promise<void> {
		if (calibrationState?.signatureStale !== true || calibrationState.staleRerunPromptShownAt !== undefined || !mainWindow || mainWindow.isDestroyed()) return;
		...
		if (validation.classification === 'recalibration-recommended') {
			...
			if (mainWindow && !mainWindow.isDestroyed()) {
				void dialog
					...
					.catch((error) => {
						console.error('Failed to show the calibration rollback notice', error);
					});
			}
			return true;
		}
	...
	ipcMain.handle('competitiveMode_getBackup', (event) => (isTrustedGameIpcSender(event) ? loadCompetitiveModeBackup() : undefined));
	...
	ipcMain.on('settingsUI_updates_userPrefs', (event, data: unknown) => {
		...
		const validUpdates = Object.fromEntries(Object.entries(parsedUpdates).filter(([key]) => Object.hasOwn(userPrefs, key)));
	...
	const CALIBRATION_TRIAL_DEADLINE_MS = WORKLOAD_CONSTANTS.warmupMaxMs + CALIBRATION_BENCHMARK_MS + 5000;
	const CALIBRATION_MAXIMIZE_GRACE_MS = 1500;
					...
					await runBeforeDeadline(
						() =>
							new Promise<void>((resolve, reject) => {
								onMaximized = resolve;
								onClosed = () => {
									reject(new Error('Calibration window was closed before maximization.'));
								};
						...
						maximizeDeadline,
						'Calibration window maximization'
					);
				} finally {
	...
	async function runCalibrationTrial(candidate: CalibrationCandidate, step: number, total: number, attempt = 1, previousAttempt?: CalibrationMetrics): Promise<CalibrationMetrics> {
		const deadlineAt = clampCalibrationTrialDeadline(calibrationState, Date.now() + CALIBRATION_TRIAL_DEADLINE_MS);
		let trialWindow: BrowserWindow | undefined;
		try {
			const [{ buildCalibrationTrialPage }, markSvg] = await runBeforeDeadline(
				() => Promise.all([import('./calibration-window.ts'), readFile(pathJoin($assets, 'wok-mark.svg'), 'utf-8')]),
				deadlineAt,
				'Calibration assets'
			);
			if (!mainWindow || mainWindow.isDestroyed()) {
				mainWindow = await runBeforeDeadline(() => createCalibrationWindow(deadlineAt), deadlineAt, 'Calibration window creation');
			}
			trialWindow = mainWindow;
			const activeTrialWindow = trialWindow;
			const { display } = getGameplayDisplay();
			const trialUrl = calibrationDataUrl(
				buildCalibrationTrialPage(candidate, step, total, markSvg, {
					attempt,
			...
			await runBeforeDeadline(() => activeTrialWindow.loadURL(trialUrl), deadlineAt, 'Calibration page navigation');
			...
			try {
				const benchmark = () =>
					activeTrialWindow.webContents.executeJavaScript(
				...
				return normalizeBenchmarkMetrics(await runBeforeDeadline(() => Promise.race([benchmark(), closed]), deadlineAt, 'Calibration renderer'));
			} finally {
	...
	async function showCalibrationDecision(state: CalibrationState): Promise<'apply' | 'keep'> {
		...
		try {
			const [{ buildCalibrationResultPage }, markSvg] = await runBeforeDeadline(
				() => Promise.all([import('./calibration-window.ts'), readFile(pathJoin($assets, 'wok-mark.svg'), 'utf-8')]),
				deadlineAt,
				'Calibration result assets'
			);
			if (!mainWindow || mainWindow.isDestroyed()) {
				mainWindow = await runBeforeDeadline(() => createCalibrationWindow(deadlineAt), deadlineAt, 'Calibration result window creation');
			}
			decisionWindow = mainWindow;
			await runBeforeDeadline(
				() => decisionWindow.loadURL(calibrationDataUrl(buildCalibrationResultPage(state.results, state.recommendedSelection, markSvg, state.competitiveModeWasEnabled))),
				deadlineAt,
				'Calibration result navigation'
			);
	...
	function prepareCalibrationForGraphicsIdentity(hardwareFingerprint: string, driverFingerprint: string): CalibrationState {
		const signature = createCalibrationSignature(app.getVersion(), process.versions.electron, hardwareFingerprint, driverFingerprint);
	...
	function prepareCalibrationForGpuInfo(gpuInfo: unknown): CalibrationState {
		return prepareCalibrationForGraphicsIdentity(graphicsProfileState.hardwareFingerprint, calibrationDriverFingerprint(gpuInfo));
	}
	...
	async function runCalibrationTrialWithRetry(candidate: CalibrationCandidate, step: number, total: number): Promise<CalibrationTrialAttemptOutcome> {
		let previousAttempt: CalibrationMetrics | undefined;
		return orchestrateCalibrationTrialRetry({
			candidate,
			getState: () => calibrationState,
			isRunTimeBudgetExhausted: (state) => isCalibrationRunTimeBudgetExhausted(state),
			...
			runAttempt: async (attempt) => {
				...
				try {
					metrics = await runCalibrationTrial(candidate, step, total, attempt, previousAttempt);
					failureReason = activeCalibrationFailureReason;
					if (failureReason) metrics = failedCalibrationMetrics();
					else if (!metrics.success) {
						failureReason = 'WebGL calibration did not return valid frame samples.';
					}
				} catch (error) {
					if (!mainWindow || mainWindow.isDestroyed()) {
				...
				if (!failureReason && powerStateChanged) {
					metrics = markMetricsRejected(metrics, 'power-state-changed');
				}
	...
	async function runCalibrationFlow(gpuInfo: unknown): Promise<boolean> {
		...
		if (calibrationState.status === 'running') {
			...
			while (pendingCandidate && isGraphicsBackendQuarantined(graphicsProfileState, pendingCandidate.backend)) {
				persistCalibrationState(recordCalibrationResult(calibrationState, pendingCandidate, failedCalibrationMetrics(), `${pendingCandidate.backend} is blocked after a previous GPU-process failure.`));
			...
			if (pendingCandidate) {
				...
				for (let trialInLaunch = 0; trialInLaunch < launchTrialCount; trialInLaunch++) {
					...
					if (trialInLaunch > 0) {
						await new Promise((resolve) => {
							setTimeout(resolve, CALIBRATION_INTRA_LAUNCH_COOLDOWN_MS);
						});
						if (finalizeCalibrationIfRunTimeBudgetExhausted()) break;
					}
		...
		try {
			...
			if (applyRecommendation && calibrationState.activeSelection) {
				...
				const retainedBackend = calibrationState.activeSelection?.candidate.backend ?? calibrationState.candidates[0]?.backend ?? graphicsSelection.backend;
				graphicsProfileState = keepCurrentGraphicsBackend(graphicsProfileState, retainedBackend, `Keeping ${retainedBackend} after the calibration recommendation was declined.`);
				persistGraphicsProfile();
			}
			relaunchClient();
		} catch (error) {
	...
	async function refreshCompleteGraphicsInfo(): Promise<unknown> {
		try {
			const gpuInfo = await runBeforeDeadline(() => fetchCalibrationGraphicsInfo((infoType) => app.getGPUInfo(infoType)), Date.now() + GPU_INFO_TIMEOUT_MS, 'Complete GPU information');
			...
			graphicsProfileState = updateGraphicsDriverIdentity(graphicsProfileState, calibrationDriverFingerprint(gpuInfo));
			persistGraphicsProfile();
	...
	async function refreshRuntimeGraphicsInfo(): Promise<boolean> {
		try {
			const gpuInfo = await runBeforeDeadline(() => fetchRuntimeGraphicsInfo((infoType) => app.getGPUInfo(infoType)), Date.now() + GPU_INFO_TIMEOUT_MS, 'Basic GPU information');
			const devices = normalizeGraphicsDevices(gpuInfo);
			graphicsProfileState = updateGraphicsDetection(graphicsProfileState, process.platform, devices);
			persistGraphicsProfile();
	...
	app.on('ready', async () => {
		...
		const bootPayloadArguments = (() => {
			try {
				...
				return payload.length <= 24000 ? [`--wok-boot=${payload}`] : [];
			} catch (error) {
		...
		const runtimeProfiler = new RuntimeProfiler({
			debugger: {
				attach: (protocolVersion) => {
					mainWindow.webContents.debugger.attach(protocolVersion);
				},
				detach: () => {
		...
		if (deferredMigrationSources.length > 0) {
			...
			void migrateLegacyConfigsPhaseTwo(configPath, migrationSources)
				.then((migration) => {
				...
				.catch((error) => {
					console.error('Failed to migrate remaining legacy Crankshaft configuration. The original files were left untouched.', error);
				});
		}
		...
		ipcMain.on('preload_updates_DiscordRPC', (event, value: unknown) => {
			...
			if (typeof data.details !== 'string' || data.details.length > 128 || typeof data.state !== 'string' || data.state.length > 128) return;
			if (discordRPCReady) {
		...
		if (userPrefs.discordRPC) {
			const startDiscordRPC = () => {
				void import('./discord-rpc.ts')
					.then(({ DiscordRpcClient }) => {
			...
			const DISCORD_RPC_START_DELAY_MS = 2000;
		...
		const introHandoff = createIntroGameWindowHandoff(mainWindow, userPrefs.fullscreen);
		...
		const GRAPHICS_STABILITY_CONFIRMATION_MS = 30000;
		...
		mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedUrl, isMainFrame) => {
			...
			if (!graphicsProfileState.launchPending) return;
			graphicsProfileState = recordUnknownGraphicsLaunchInterruption(graphicsProfileState, `Main-frame navigation to ${validatedUrl || 'Krunker'} failed (${errorCode}: ${errorDescription || 'unknown error'}).`);
			persistGraphicsProfile();
		});
		mainWindow.webContents.on('did-finish-load', () => {
			...
			const currentAdaptiveValidationState = userPrefs.competitiveMode ? prepareCurrentAdaptiveValidationState() : undefined;
		...
		const gameSubmenu: MenuItemConstructorOptions | MenuItem = {
			...
			submenu: [
				...
				{
					label: 'Copy game link to clipboard',
					accelerator: 'F7',
					click: () => {
		...
		if (!calibrationBlocksStartup) {
			const GPU_IDENTITY_REFRESH_DELAY_MS = 15000;
			...
			mainWindow.webContents.once('did-finish-load', () => {
				gpuIdentityRefreshTimer = setTimeout(() => {
					...
					void refreshRuntimeGraphicsInfo().then((refreshed) => {
						if (!refreshed) return;
						if (graphicsProfileState.hardwareFingerprint !== '' && graphicsProfileState.driverFingerprint !== '') {
							const previousCalibrationState = calibrationState;
							const preparedState = prepareCalibrationForGraphicsIdentity(graphicsProfileState.hardwareFingerprint, graphicsProfileState.driverFingerprint);
							if (previousCalibrationState && preparedState !== previousCalibrationState && preparedState.status === 'uncalibrated') {
		...
		const revealDeadline = startLoadingDeadline({
			...
			onResolve: (resolution) => {
				...
				const readiness = gameUsableReportedAt === undefined ? 'never' : `after ${Math.round(gameUsableReportedAt - processStartWallClockMs)}ms`;
