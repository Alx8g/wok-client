...
const lazyBoundaries = ['src/settingsui.ts', 'src/matchmaker.ts', 'src/performance-monitor.ts', 'src/discord-rpc.ts', 'src/competitive-mode.ts', 'src/adaptive-validation-runtime.ts', 'src/calibration-window.ts'];
...
const calibrationWindowOutput = Object.keys(metafile.outputs).find((outputPath) => Object.keys(metafile.outputs[outputPath].inputs).some((inputPath) => inputPath.replaceAll('\\', '/') === 'src/calibration-window.ts'));
...
assert.equal(typeof bundledCalibrationWindow.buildCalibrationTrialPage, 'function', `${calibrationWindowOutput} does not export buildCalibrationTrialPage`);
const page = bundledCalibrationWindow.buildCalibrationTrialPage({ backend: 'default', framePolicy: 'uncapped', id: 'default:uncapped' }, 1, 1, '<svg></svg>');
...
assert.equal(embedded.createWorkloadSpin(WORKLOAD_SEED, 500)(), createWorkloadSpin(WORKLOAD_SEED, 500)(), 'embedded createWorkloadSpin diverged from src');
function createRecordingGl() {
	...
	const serializeArg = (value) => {
		if (value instanceof Float32Array || value instanceof Uint16Array || value instanceof Uint8Array) {
			let hash = 0;
			for (let index = 0; index < value.length; index++) hash = Math.imul(hash ^ Math.round(value[index] * 1000), 0x01000193) >>> 0;
...
function renderDigest(createWorkloadImplementation) {
	...
	const workload = createWorkloadImplementation(recording.gl, createWorkloadSpec(WORKLOAD_SEED), 1920, 1080);
	for (let frameIndex = 0; frameIndex < 2; frameIndex++) workload.renderFrame(frameIndex);
	return commandStreamDigest(recording.calls);
}
assert.equal(renderDigest(embedded.createWorkload), renderDigest(createWorkload), 'the bundled trial page workload no longer emits the identical command stream as src');
