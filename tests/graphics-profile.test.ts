...
test('normalizes and orders Chromium GPU devices', () => {
	const devices = normalizeGraphicsDevices({
		gpuDevice: [nvidiaDevice, intelDevice, { active: true, deviceId: 'invalid', vendorId: 1 }]
	});
...
test('hardware identity ignores adapter activity and enumeration order', () => {
	...
	assert.equal(graphicsHardwareFingerprint([activeIntel, inactiveNvidia]), graphicsHardwareFingerprint([activeNvidia, inactiveIntel]));
});
...
test('an unknown or network interruption is not classified as a GPU failure', () => {
	let state = intelGraphicsState();
	state = beginGraphicsLaunch(state, selectGraphicsBackend('auto', state, 'win32', START_TIME), START_TIME + 1);
...
test('a confirmed GPU-process failure immediately selects a safe fallback', () => {
	let state = intelGraphicsState();
	state = beginGraphicsLaunch(state, selectGraphicsBackend('auto', state, 'win32', START_TIME), START_TIME + 1);
	state = recordGraphicsGpuFailure(state, 'd3d11on12', 'GPU process crashed with exit code 34.', START_TIME + 2);
	assert.equal(state.launchPending, false);
...
test('GPU failure quarantine expires and a successful retry clears backend failure history', () => {
	let state = recordGraphicsGpuFailure(intelGraphicsState(), 'd3d11on12', 'GPU process launch-failed.', START_TIME);
...
test('keeping the current backend overrides an unaccepted automatic recommendation', () => {
	let state = intelGraphicsState();
	state = keepCurrentGraphicsBackend(state, 'default', 'Keeping Chromium default after rejecting the calibration recommendation.', START_TIME + 1);
