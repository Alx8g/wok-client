...
import {
	MatchmakerResponseTooLargeError,
	readBoundedMatchmakerJson
} from '../src/matchmaker-response.ts';
...
test('parses a chunked response at the exact byte limit', async () => {
	...
	assert.deepEqual(
		await readBoundedMatchmakerJson(response, first.byteLength + second.byteLength),
		{ games: [] }
	);
});
...
test('cancels a chunked response as soon as it exceeds the byte limit', async () => {
	...
	await assert.rejects(
		readBoundedMatchmakerJson(response, 12),
		MatchmakerResponseTooLargeError
	);
...
test('rejects an oversized declared content length before reading', async () => {
	...
	await assert.rejects(
		readBoundedMatchmakerJson(response, 64),
		MatchmakerResponseTooLargeError
	);
...
test('propagates malformed JSON without allocating beyond the bound', async () => {
	const body = '{"games":[';
	await assert.rejects(
		readBoundedMatchmakerJson(new Response(body), Buffer.byteLength(body)),
		SyntaxError
	);
});
...
test('rejects unavailable bodies and invalid limits', async () => {
	await assert.rejects(
		readBoundedMatchmakerJson(new Response(null), 64),
		/response body is unavailable/u
	);
	await assert.rejects(
		readBoundedMatchmakerJson(new Response('{}'), 0),
		RangeError
	);
});
