...
import { createIntroGameWindowHandoff, getIntroWindowBounds, selectIntroSource, startIntroSequence, type IntroSequenceScheduler } from '../src/intro-window.ts';
class FakeScheduler implements IntroSequenceScheduler {
	...
	public runDelay(delayMs: number): void {
		const match = [...this.scheduled.entries()].find(([, entry]) => entry.delayMs === delayMs);
...
class FakeIntroWindow extends EventEmitter {
	...
	public setAlwaysOnTop(flag: boolean, level: string): void {
...
test('intro bounds and source selection follow the game geometry and physical pixels', () => {
	...
	assert.deepEqual(getIntroWindowBounds(display, 'fullscreen', 0.75), display.bounds);
	assert.deepEqual(getIntroWindowBounds(display, 'maximized', 0.75), display.workArea);
	const windowed = getIntroWindowBounds(display, 'windowed', 0.75);
	...
	assert.equal(selectIntroSource({ ...display, scaleFactor: 1.25 }, windowed), '1440');
});
test('intro startup, playback, visual handoff and audio-tail completion run once in order', () => {
	...
	assert.deepEqual(
		{
			...
			sandbox: harness.constructorOptions?.webPreferences?.sandbox,
	...
	assert.deepEqual(harness.introWindow.alwaysOnTop, [true, 'screen-saver']);
	...
	harness.introWindow.webContents.emit('media-started-playing');
	harness.scheduler.runDelay(1450);
	assert.deepEqual(harness.callbackOrder, ['reveal']);
	assert.equal(harness.introWindow.visible, true);
	harness.scheduler.runDelay(3000);
	assert.deepEqual(harness.callbackOrder, ['reveal', 'visual-end']);
	assert.equal(harness.introWindow.visible, false);
	assert.equal(harness.introWindow.destroyed, false);
	harness.scheduler.runDelay(4250);
	assert.deepEqual(harness.callbackOrder, ['reveal', 'visual-end', 'finished']);
...
test('intro media-start timeout fails forward and clears every remaining timer', () => {
	...
	harness.introWindow.emit('ready-to-show');
	harness.scheduler.runDelay(2000);
	assert.deepEqual(harness.callbackOrder, ['reveal', 'visual-end', 'finished']);
	assert.equal(harness.introWindow.destroyed, true);
	assert.equal(harness.scheduler.size, 0);
});
test('closing or cancelling the intro completes all callbacks exactly once', () => {
	...
	assert.deepEqual(closed.callbackOrder, ['reveal', 'visual-end', 'finished']);
	...
	assert.deepEqual(cancelled.callbackOrder, ['reveal', 'visual-end', 'finished']);
...
test('a window operation that throws mid-sequence cannot strand the handoff', () => {
	...
	hideFailure.scheduler.runDelay(1450);
	hideFailure.scheduler.runDelay(3000);
	assert.deepEqual(hideFailure.callbackOrder, ['reveal', 'visual-end']);
	hideFailure.scheduler.runDelay(4250);
	assert.deepEqual(hideFailure.callbackOrder, ['reveal', 'visual-end', 'finished']);
	...
	assert.deepEqual(destroyedEarly.callbackOrder, ['reveal', 'visual-end', 'finished']);
...
test('game readiness stays behind the intro then receives focus at the visual handoff', () => {
	...
	assert.deepEqual(calls, ['maximize', 'show-inactive', 'focus']);
	handoff.handleReadyToShow();
	assert.deepEqual(calls, ['maximize', 'show-inactive', 'focus', 'focus']);
	destroyed = true;
	handoff.revealForUse();
