...
import { calculateFrameRetention, calculateMouseMotionFactor, motionBlurOptionsFromUserPrefs } from '../src/motion-blur.ts';
...
test('ignores fine aim corrections and normalizes turn speed across frame rates', () => {
	assert.equal(calculateMouseMotionFactor(2.9, 1000 / 60), 0);
	assert.equal(calculateMouseMotionFactor(30, 1000 / 60), 1);
	assert.equal(calculateMouseMotionFactor(15, 1000 / 120), 1);
	assert.ok(calculateMouseMotionFactor(10, 1000 / 60) < 0.2);
});
test('keeps temporal frame retention consistent across frame rates', () => {
	const sixtyFpsRetention = calculateFrameRetention(0.48, 1000 / 60);
	const thirtyFpsRetention = calculateFrameRetention(0.48, 1000 / 30);
