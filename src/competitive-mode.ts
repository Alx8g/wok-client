...
import {
	...
	createChangedSettingsPatch,
	type CompetitiveGameSettings
} from './competitive-settings.ts';
...
function waitForGameSettings(timeoutMs = 15_000): Promise<void> {
...
function findSettingControl(root: HTMLElement, key: string): HTMLInputElement | HTMLSelectElement | undefined {
	...
	return [...root.querySelectorAll<HTMLInputElement | HTMLSelectElement>('input, select')]
		.find(element => {
			const handlers = ['onclick', 'onchange', 'oninput', 'onkeypress']
...
async function enableCompetitiveGameSettings() {
	...
	const reversibleSettings = Object.fromEntries(
		Object.entries(COMPETITIVE_GAME_SETTINGS).filter(([key]) => Object.hasOwn(backup.settings, key))
	);
