...
import { MATCHMAKER_GAMEMODES, MATCHMAKER_MAP_ICON_INDICES } from './matchmaker-data.ts';
import { matchmakerCandidateRegions, waitForMatchmakerOperation } from './matchmaker-flow.ts';
...
import { collectMatchmakerCandidates, matchmakerRegionLatency, rankMatchmakerCandidates, selectRevalidatedMatchmakerGame } from './matchmaker-selection.ts';
import { createElement, keyboardEventMatchesCustomSetting, parseKeybindSettingDisplay } from './utils.ts';
export { MATCHMAKER_GAMEMODES, MATCHMAKER_MAP_ICON_INDICES, MATCHMAKER_REGION_NAMES, MATCHMAKER_REGIONS } from './matchmaker-data.ts';
const MATCHMAKER_REQUEST_TIMEOUT_MS = 10000;
const MATCHMAKER_MINIMUM_SEARCH_VISIBLE_MS = 750;
const MATCHMAKER_GAME_LIST_URL = 'https://matchmaker.krunker.io/game-list';
const MATCHMAKER_CANCELLED_POPUP_MS = 1400;
...
function createMatchmakerButton(id: string, accept: boolean): MatchmakerPopupButton {
	...
	button.addEventListener('click', () => {
		decideMatchmakerDecision(accept);
	});
...
function startMatchmakerTicker(tick: () => void) {
	stopMatchmakerTicker();
	tick();
	popupTicker = window.setInterval(tick, 1000);
}
...
function showMatchmakerLobbyPopup(game: IMatchmakerGame, latencyMs: number | undefined) {
	...
	popupElement.style.backgroundImage = mapIndex >= 0 ? `url(https://assets.krunker.io/img/maps/map_${mapIndex}.png)` : '';
	currentMatch = game.gameID;
	if (!showMatchmakerPopup(matchmakerLobbyView(game, latencyMs, hotkeyLabels))) return;
	const endsAt = Date.now() + game.remainingTime * 1000;
...
function showMatchmakerSearchingPopup(criteria: IMatchmakerCriteria): boolean {
	...
	startMatchmakerTicker(() => {
		popupTimer.textContent = matchmakerDurationLabel(Math.floor((Date.now() - startedAt) / 1000));
	});
	return true;
}
...
async function loadMatchmakerGameList(request: AbortController): Promise<unknown> {
	...
	if (!response.ok) {
		throw new MatchmakerHttpError(response.status, response.status === 429 ? response.headers.get('Retry-After') : null);
	}
...
function matchmakerHotkeyLabel(value: unknown, fallback: string): string {
	...
	return pref && typeof pref.key === 'string' && pref.key.length > 0 ? parseKeybindSettingDisplay(pref) : fallback;
}
export async function fetchGame(_userPrefs: UserPrefs) {
	...
	try {
		...
		const candidates = collectMatchmakerCandidates(initialResult, criteria, selectionContext, MATCHMAKER_GAMEMODES);
		...
		try {
			latencyResult = await waitForMatchmakerOperation(request.signal, () => ipcRenderer.invoke('matchmaker_measure_region_latency', matchmakerCandidateRegions(candidates)));
			assertCurrentMatchmakerRequest(request);
		} catch (error) {
			assertCurrentMatchmakerRequest(request);
		...
		const freshCandidates = collectMatchmakerCandidates(freshResult, criteria, selectionContext, MATCHMAKER_GAMEMODES);
		const selectedGame = selectRevalidatedMatchmakerGame(rankedCandidates, freshCandidates);
		...
		showMatchmakerErrorPopup(
			error instanceof MatchmakerResponseTooLargeError
