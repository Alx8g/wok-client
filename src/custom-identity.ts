...
const IDENTITY_PREFERENCE_KEYS = new Set<string>([CUSTOM_CLAN_PREFERENCE_KEY, CUSTOM_NAME_PREFERENCE_KEY, REAL_CLAN_PREFERENCE_KEY, REAL_NAME_PREFERENCE_KEY]);
...
export function isPlausibleRealName(value: unknown): value is string {
	return typeof value === 'string' && PLAUSIBLE_REAL_NAME.test(value) && !isPlaceholderRealName(value);
}
...
export function extractClanTag(text: unknown, name: string): string {
	...
	const pattern = new RegExp(`\\[([A-Za-z0-9_-]{1,${CUSTOM_CLAN_MAX_LENGTH}})\\]\\s*${name.replace(/[.*+?^${}()|[\]\\]/gu, '\\$&')}(?![A-Za-z0-9_-])`, 'u');
...
export function mergeRealIdentityCandidates(configured: Readonly<CustomIdentity>, discovered: Readonly<Partial<CustomIdentity>>): RealIdentityCandidates {
	const collect = (...values: (string | undefined)[]) => [...new Set(values.filter((value): value is string => typeof value === 'string' && value !== ''))];
	return {
