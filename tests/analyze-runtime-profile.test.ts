...
const profile = {
	...
	samples: [1, 2, 1, 3],
	startTime: 0,
	timeDeltas: [1000, 2000, 1000, 6000]
};
test('ranks CPU-profile nodes by measured self-time and separates runtime categories', () => {
	...
	assert.deepEqual(
		analysis.topInclusive.slice(0, 2).map((entry) => [entry.functionName, entry.inclusiveMs, entry.inclusivePercent]),
		[
			['gameLoop', 8, 80],
			['wokHook', 6, 60]
		]
...
test('falls back to profile duration when recorded sample deltas are unavailable', () => {
	...
	assert.equal(analysis.attributedMs, 10);
	assert.deepEqual(
		analysis.top.map((entry) => [entry.functionName, entry.selfMs]),
		[
			['gameLoop', 5],
