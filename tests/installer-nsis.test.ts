/** biome-ignore-all lint/suspicious/noTemplateCurlyInString: asserting on nsis scripting language templating */
...
import {
	...
	INSTALLER_ART_SIZES,
	renderInstallerArt
} from '../scripts/generate-installer-art.mjs';
...
test('silent installs get the same payload, with switches to drop the optional shortcuts', () => {
	// A "/o" section would be unselected by default, so /S would silently skip it.
...
test('the progress log names each install step', () => {
	...
	for (const step of [
		'Checking whether ${WOK_NAME} is running...',
		'Copying ${WOK_NAME} to $INSTDIR...',
		'Writing the uninstaller...',
		'Registering ${WOK_NAME} with Windows...',
		'Recording the installed size...'
	]) {
		assert.ok(details.includes(step), `missing install progress step: ${step}`);
	}
	for (const step of [
		'Removing shortcuts...',
		'Removing the Windows registration...',
		'Removing application files from $INSTDIR...'
	]) {
...
test('the uninstaller removes what the installer created and keeps user settings', () => {
	...
	// $INSTDIR is only wiped when it still looks like our installation.
...
test('the committed installer bitmaps match the generator output byte for byte', () => {
	for (const [name, bytes] of renderInstallerArt()) {
		...
		assert.ok(
			...
			`${name} is stale. Run: node scripts/generate-installer-art.mjs`
		);
	}
});
