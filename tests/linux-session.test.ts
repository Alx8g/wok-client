...
import {
	OZONE_OVERRIDE_ENV_VAR,
	describeLinuxDisplaySession,
	linuxLauncherArguments,
	ozoneSwitchArguments,
	renderLinuxLauncherScript,
	resolveOzonePlatform
} from '../src/linux-session.ts';
...
test('WAYLAND_DISPLAY alone is enough, which is the gap in Electron\'s own detection', () => {
	// Chromium's SetOzonePlatformForLinuxIfNeeded reads XDG_SESSION_TYPE and nothing else, so this
	// environment - a TTY or container launch inside a live Wayland session - would land on X11.
...
test('the override is matched exactly, the same way the generated launcher matches it', () => {
	// A POSIX `case` cannot fold case portably, so the resolver does not either: the escape hatch
	// has to mean the same thing in the packaged app and in development.
...
test('the generated launcher execs the real binary and forwards its arguments', () => {
	...
	// Every exec path must forward "$@": dropping it would break wok:// and crankshaft:// links.
...
test('the startup line honours the last --ozone-platform, as Chromium does', () => {
	const line = describeLinuxDisplaySession(
		'linux',
		['wok-client', '--ozone-platform=wayland', '--ozone-platform=x11'],
...
/**
 * The launcher is generated shell, so the only honest check is to run it. A stub stands in for the
 * packaged binary and prints the argv it was handed, which is compared against the TypeScript
 * resolver for the same environment. Skipped where there is no POSIX shell (a plain Windows
 * checkout); the Linux and macOS validation jobs execute it.
 */
...
test('the generated launcher agrees with the resolver', { skip: shellAvailable ? false : 'no POSIX sh' }, () => {
	...
	try {
		...
		for (const environment of environments) {
			...
			assert.deepEqual(
				argv,
				[...linuxLauncherArguments(environment, 'linux'), '--passthrough'],
				`launcher disagreed with the resolver for ${JSON.stringify(environment)}`
			);
		}
	} finally {
