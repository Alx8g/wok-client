...
const packagedAssetNames = [
	...
	'wok-mark.svg',
	'hideAds.css',
	'intro-short-1080.webm',
	'intro-short-1440.webm',
	'intro-long-1080.webm',
	'intro-long-1440.webm',
	'intro.html',
	'intro.js',
	'matchmaker.css',
	'menuTimer.css',
	'quickClassPicker.css',
	'settings.css',
	'splash-frame.webp',
	'splash.css',
	...THEME_LAYER_ASSETS,
	...BUNDLED_THEMES.map((theme) => themeAssetName(theme.id))
];
const packagedAssetsPattern = new RegExp(`^/assets/(?!(?:${packagedAssetNames.map((name) => name.replaceAll('.', '\\.')).join('|')})$)`);
...
function copyWokNotices(buildPath: string, platform: string) {
	...
	if (platform === 'darwin') {
		const appBundle = readdirSync(buildPath, { withFileTypes: true }).find((entry) => entry.isDirectory() && entry.name.endsWith('.app'));
...
export default {
	packagerConfig: {
		...
		ignore: [/^\/(?!(bundle|assets|node_modules|package\.json|LICENSE|THIRD_PARTY_NOTICES\.txt|PATCHED_ELECTRON\.txt))/, /^\/bundle\/metafile\.json$/, /^\/bundle\/.*\.mjs\.map$/, packagedAssetsPattern],
		prune: true,
		asar: true,
		download: {
			mirrorOptions: {
	...
	hooks: {
		...
		postPackage: async (config, { platform, outputPaths }) => {
			for (const buildPath of outputPaths) {
				...
				console.log(`Verified packaged ASAR: ${verification.bundleOutputCount} bundle outputs, ` + `${verification.assetCount} assets.`);
			}
		},
