...
function deviceLine(profile: GraphicsProfileState): string {
	...
	return profile.devices.map((device) => `${device.vendorId.toString(16)}:${device.deviceId.toString(16)}${device.active ? ' (active)' : ''}`).join(', ');
}
...
function validationSection(validation: AdaptiveValidationState | undefined): string[] {
	...
	const header =
		`VALIDATION: ${validation.status} (${validation.sessions.length}/3) — ${validation.classification}` +
	...
	validation.sessions.forEach((session, index) => {
		...
		lines.push(
			`  session ${index + 1}: avg ${metrics.averageFps.toFixed(1)}  1%low ${metrics.onePercentLowFps.toFixed(1)}  p95 ${metrics.p95FrameTimeMs.toFixed(2)}ms  (${Math.round(session.durationMs / 1000)}s, ${metrics.sampleCount} samples)`
		);
	});
	return lines;
}
export function buildDiagnosticsReport(input: DiagnosticsReportInput): string {
	...
	const backendFailures = profile.backendFailures.length > 0 ? profile.backendFailures.map((failure) => `${failure.backend} x${failure.failureCount}${failure.reason ? ` — ${failure.reason}` : ''}`).join('; ') : 'none';
	const features =
		Object.entries(input.gpuFeatureStatus)
			.filter(([key]) => key === 'webgl' || key === 'webgl2' || key === 'gpu_compositing' || key === 'rasterization')
			.map(([key, value]) => `${key}=${value}`)
			.join(' ') || 'not reported yet';
	const preferences = REPORTED_PREFERENCE_KEYS.map((key) => `${key}=${String(input.preferences[key] ?? 'unset')}`).join(' ');
	const lines = [
