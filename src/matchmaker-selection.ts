import { MATCHMAKER_OFFICIAL_MAPS, MATCHMAKER_REGIONS, normalizeMatchmakerMapIdentifier } from './matchmaker-data.ts';
export const MATCHMAKER_MAX_RESPONSE_GAMES = 10000;
...
function allowedMatchmakerMapIdentifiers(criteria: IMatchmakerCriteria): ReadonlySet<string> | undefined {
	...
	const selectedIdentifiers = new Set(criteria.maps.map(normalizeMatchmakerMapIdentifier).filter((identifier) => identifier.length > 0));
	if (criteria.mapScope === 'official') {
		if (selectedIdentifiers.size === 0) return OFFICIAL_MATCHMAKER_MAP_IDENTIFIERS;
		return new Set([...selectedIdentifiers].filter((identifier) => OFFICIAL_MATCHMAKER_MAP_IDENTIFIERS.has(identifier)));
	}
...
const MATCHMAKER_MAX_VALID_LATENCY_MS = 60000;
interface RankedMatchmakerCandidate {
	game: IMatchmakerGame;
	index: number;
	latencyMs?: number;
}
export function matchmakerRegionLatency(latencies: Readonly<Record<string, unknown>>, region: string): number | undefined {
	if (!Object.prototype.hasOwnProperty.call(latencies, region)) return undefined;
	const value = latencies[region];
	return typeof value === 'number' && Number.isFinite(value) && value >= 0 && value <= MATCHMAKER_MAX_VALID_LATENCY_MS ? value : undefined;
}
function compareRankedMatchmakerCandidates(left: RankedMatchmakerCandidate, right: RankedMatchmakerCandidate): number {
	...
	return right.game.playerCount - left.game.playerCount || right.game.remainingTime - left.game.remainingTime || left.index - right.index;
}
function isComparableMatchmakerCandidate(candidate: RankedMatchmakerCandidate, best: RankedMatchmakerCandidate): boolean {
	const comparableLatency = best.latencyMs === undefined ? candidate.latencyMs === undefined : candidate.latencyMs !== undefined && candidate.latencyMs - best.latencyMs <= MATCHMAKER_COMPARABLE_LATENCY_MS;
	return (
		comparableLatency &&
...
export function rankMatchmakerCandidates(candidates: readonly IMatchmakerGame[], latencies: Readonly<Record<string, unknown>>, random: () => number = Math.random): IMatchmakerGame[] {
	const ranked = candidates.map(
		(game, index): RankedMatchmakerCandidate => ({
			game,
			index,
	...
	const best = ranked[0];
	const comparable = ranked.filter((candidate) => isComparableMatchmakerCandidate(candidate, best)).slice(0, MATCHMAKER_COMPARABLE_POOL_LIMIT);
	if (comparable.length < 2) return ranked.map((candidate) => candidate.game);
	const sample = random();
	const normalizedSample = Number.isFinite(sample) ? Math.min(Math.max(sample, 0), 1 - Number.EPSILON) : 0;
	...
	return [selected.game, ...ranked.filter((candidate) => candidate !== selected).map((candidate) => candidate.game)];
}
export function selectRevalidatedMatchmakerGame(rankedCandidates: readonly IMatchmakerGame[], freshCandidates: readonly IMatchmakerGame[]): IMatchmakerGame | undefined {
...
export function collectMatchmakerCandidates(value: unknown, criteria: IMatchmakerCriteria, context: MatchmakerSelectionContext, gameModes: readonly string[]): IMatchmakerGame[] {
...
export function selectMatchmakerGame(value: unknown, criteria: IMatchmakerCriteria, context: MatchmakerSelectionContext, gameModes: readonly string[], random: () => number = Math.random): IMatchmakerGame | undefined {
