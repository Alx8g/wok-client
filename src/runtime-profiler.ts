...
function validateRequest(request: RuntimeProfileRequest): void {
	...
	if (
		!nonEmptyString(request.paths.cpuProfile)
		|| !nonEmptyString(request.paths.manifest)
		|| !nonEmptyString(request.paths.trace)
	) {
		throw new Error('Runtime profile artifact paths must be non-empty.');
	}
	if (request.traceCategories.length === 0 || request.traceCategories.some(category => !nonEmptyString(category))) {
		throw new Error('Runtime profile trace categories must contain non-empty values.');
	}
}
...
export class RuntimeProfiler {
	...
	public async capture(request: RuntimeProfileRequest): Promise<RuntimeProfileResult> {
		...
		try {
			...
			await this.environment.startTracing(request.traceCategories);
			tracingStarted = true;
			...
			const tracePath = await this.environment.stopTracing(request.paths.trace);
			...
			await this.environment.writeJson(request.paths.manifest, {
				...
				traceCategories: request.traceCategories,
				tracePath
			});
			...
			if (tracingStarted) {
				try {
					await this.environment.stopTracing(request.paths.trace);
				} catch (cleanupError) {
					if (!primaryError) primaryError = cleanupError;
				}
			}
