...
test('long filter selections are summarised rather than truncated away', () => {
	const chips = matchmakerFilterChips(
		criteria({
...
test('an empty result names the filter most likely to be the cause', () => {
	assert.equal(matchmakerNoResultsHint(criteria({ mapScope: 'selected', maps: [] })), 'No maps are selected.');
	assert.equal(matchmakerNoResultsHint(criteria({ maxPlayers: 1, minPlayers: 5 })), 'Minimum players is above maximum.');
	assert.equal(matchmakerNoResultsHint(criteria({ mapScope: 'selected', maps: ['Burg', 'Site'] })), 'Try selecting more maps.');
	assert.equal(matchmakerNoResultsHint(criteria({ mapScope: 'official', maps: ['Burg'] })), 'Try selecting more maps.');
	...
	assert.equal(matchmakerNoResultsHint(criteria({ minRemainingTime: 300 })), 'Try lowering minimum time left.');
	assert.equal(matchmakerNoResultsHint(criteria({ maxPlayers: 4, minPlayers: 4 })), 'Try a wider player range.');
});
