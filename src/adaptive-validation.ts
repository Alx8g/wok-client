...
export const ADAPTIVE_VALIDATION_MIN_SESSION_MS = 30000;
...
export const ADAPTIVE_VALIDATION_LOW_CONFIDENCE_REASONS = ['window-blurred', 'document-visibility-changed', 'window-resized', 'severe-event-loop-disturbance'] as const;
...
export function adaptiveValidationWatchesProfile(state: AdaptiveValidationState, activeBackend: AppliedGraphicsBackend, framePolicy: AdaptiveValidationFramePolicy): boolean {
	return state.profile.activeBackend === activeBackend && state.profile.framePolicy === framePolicy;
}
export function adaptiveValidationProfileIdentitiesEqual(left: AdaptiveValidationProfileIdentity, right: AdaptiveValidationProfileIdentity): boolean {
	return (
		left.activeBackend === right.activeBackend &&
...
function parseSessionMetrics(value: unknown): AdaptiveValidationSessionMetrics | undefined {
	...
	const averageFps = finiteNumberInRange(value.averageFps, 100000);
	const onePercentLowFps = finiteNumberInRange(value.onePercentLowFps, 100000);
	const p95FrameTimeMs = finiteNumberInRange(value.p95FrameTimeMs, 60000);
	const worstFrameTimeMs = finiteNumberInRange(value.worstFrameTimeMs, 60000);
	if (averageFps === undefined || onePercentLowFps === undefined || p95FrameTimeMs === undefined || worstFrameTimeMs === undefined) return undefined;
	if (!Number.isSafeInteger(value.sampleCount) || Number(value.sampleCount) < 0 || Number(value.sampleCount) > 100000000) return undefined;
	return {
		averageFps,
		onePercentLowFps,
		p95FrameTimeMs,
...
export function parseAdaptiveValidationSession(value: unknown): AdaptiveValidationSession | undefined {
	...
	const durationMs = finiteNumberInRange(value.durationMs, 7 * 24 * 60 * 60 * 1000);
...
function sessionHasEnoughEvidence(session: AdaptiveValidationSession): boolean {
	return session.metrics.sampleCount >= ADAPTIVE_VALIDATION_MIN_FRAME_SAMPLES && session.metrics.averageFps > 0 && session.metrics.onePercentLowFps > 0 && session.metrics.p95FrameTimeMs > 0;
}
function sessionQualifies(session: AdaptiveValidationSession): boolean {
	return session.durationMs >= ADAPTIVE_VALIDATION_MIN_SESSION_MS && session.lowConfidenceReasons.length === 0 && sessionHasEnoughEvidence(session);
}
export function adaptiveValidationSessionHasSevereInstability(session: AdaptiveValidationSession): boolean {
	...
	return session.metrics.p95FrameTimeMs > ADAPTIVE_VALIDATION_SEVERE_P95_FRAME_TIME_MS || session.metrics.onePercentLowFps < ADAPTIVE_VALIDATION_SEVERE_ONE_PERCENT_LOW_FPS;
}
function medianSessionAverageFps(sessions: readonly AdaptiveValidationSession[]): number {
	...
	return sorted.length % 2 === 1 ? sorted[middle] : (sorted[Math.floor(middle)] + sorted[Math.ceil(middle)]) / 2;
}
function sessionsRegressFromBaseline(sessions: readonly AdaptiveValidationSession[], baseline: AdaptiveValidationBaseline | undefined): boolean {
...
function classificationForSessions(sessions: readonly AdaptiveValidationSession[], baseline?: AdaptiveValidationBaseline): AdaptiveValidationClassification {
...
export function createAdaptiveValidationState(profile: AdaptiveValidationProfileIdentity, now: number = Date.now(), baseline?: AdaptiveValidationBaseline): AdaptiveValidationState {
	return {
		...(baseline ? { baseline } : {}),
...
function baselineComparableToProfile(baselineProfile: AdaptiveValidationProfileIdentity, profile: AdaptiveValidationProfileIdentity): boolean {
	return (
		baselineProfile.hardwareFingerprint === profile.hardwareFingerprint &&
...
function baselineForProfileChange(existing: AdaptiveValidationState, profile: AdaptiveValidationProfileIdentity): AdaptiveValidationBaseline | undefined {
	if (existing.status === 'complete' && existing.classification === 'validated' && baselineComparableToProfile(existing.profile, profile)) {
		return {
...
export function prepareAdaptiveValidationState(existing: AdaptiveValidationState | undefined, profile: AdaptiveValidationProfileIdentity, now: number = Date.now()): AdaptiveValidationState {
...
export function recordAdaptiveValidationSession(state: AdaptiveValidationState, value: unknown, now: number = Date.now()): AdaptiveValidationState {
	...
	if (!session || !sessionQualifies(session) || state.sessions.some((existing) => existing.id === session.id) || (previousSession !== undefined && session.completedAt <= previousSession.completedAt)) return state;
...
export function dismissAdaptiveValidationRecommendation(state: AdaptiveValidationState, now: number = Date.now()): AdaptiveValidationState {
	if (state.status !== 'complete' || state.classification !== 'recalibration-recommended' || state.recommendationDismissedAt !== undefined) return state;
	return {
		...state,
...
function parseAdaptiveValidationBaseline(value: unknown): AdaptiveValidationBaseline | undefined {
	if (!isRecord(value)) return undefined;
	const medianAverageFps = finiteNumberInRange(value.medianAverageFps, 100000);
	const profile = parseAdaptiveValidationProfileIdentity(value.profile);
...
export function parseAdaptiveValidationState(value: unknown): AdaptiveValidationState | undefined {
	...
	if (new Set(parsedSessions.map((session) => session.id)).size !== parsedSessions.length || parsedSessions.some((session, index) => index > 0 && session.completedAt <= parsedSessions[index - 1].completedAt))
		return undefined;
