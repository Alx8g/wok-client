...
import { buildDisplayOptions, describeDisplay, DISPLAY_PREFERENCE_AUTO, displayKey, displayName, displayNameSlug, isDisplayPreference, selectGameplayDisplay, type SelectableDisplay } from '../src/display-selection.ts';
...
test('enumeration failing entirely still yields a usable automatic-only dropdown', () => {
	assert.deepEqual(buildDisplayOptions([], -1, DISPLAY_PREFERENCE_AUTO), [{ value: 'auto', label: 'Automatic (primary display)' }]);
});
