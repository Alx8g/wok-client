...
test('computes per-mark medians across multiple captured runs', () => {
	...
	assert.deepEqual(Object.keys(marks).sort(), ['app-ready', 'did-finish-load', 'loadurl-called', 'main-module-eval-start', 'window-created']);
...
test('ignores non-mark lines, malformed values, and trailing garbage', () => {
	const { marks } = parsePerfMarks(['[wok-mark] bad-value notanumber', '[wok-mark] trailing-garbage 12.5 extra tokens', 'plain log line', '[wok-mark] good 5'].join('\n'));
