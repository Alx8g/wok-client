import { parseKrunkerRegionCode, parseKrunkerReportedPing, parseKrunkerReportedTps, RollingReportedPingStats } from './network-diagnostics.ts';
...
const REPORTED_PING_STALE_MS = 15000;
...
function ensureNetworkObservation(now = performance.now()) {
	...
	const observer = new MutationObserver(() => {
		if (observer !== pingObserver || pingElement !== observedPingElement || !monitorStarted || !overlayVisible || document.visibilityState !== 'visible') return;
...
function renderOverlay(now = performance.now()) {
	...
	const reportedPing = networkSnapshot.available ? `${networkSnapshot.currentReportedPingMs.toFixed(0)} ms${networkSnapshot.reportedPingSampleAgeMs > REPORTED_PING_STALE_MS ? ' (stale)' : ''}` : 'collecting';
	const reportedPingRange = networkSnapshot.available ? `${networkSnapshot.medianReportedPingMs.toFixed(0)} / ${networkSnapshot.p95ReportedPingMs.toFixed(0)} ms` : 'collecting';
	const reportedPingVariation = networkSnapshot.available ? `${networkSnapshot.reportedPingVariationMs.toFixed(1)} ms` : 'collecting';
	...
	overlay.textContent = [
		...
		...(runtimeInfo.gpuAdvisory ? [`GPU WARNING   ${runtimeInfo.gpuAdvisoryKind === 'manual-backend-failure' ? 'manual backend crashing, never quarantined' : 'integrated GPU active, discrete GPU idle'}`] : []),
		`WebGL         ${webglStatus}`,
		`raster        ${rasterStatus}`,
