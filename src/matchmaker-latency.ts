...
export const MATCHMAKER_LATENCY_CACHE_TTL_MS = 60000;
export const MATCHMAKER_LATENCY_FETCH_TIMEOUT_MS = 3000;
export const MATCHMAKER_LATENCY_PROBE_TIMEOUT_MS = 1500;
...
export const MATCHMAKER_LATENCY_TARGET_STALE_TTL_MS = 5 * 60000;
export const MATCHMAKER_LATENCY_DEAD_PORT_TTL_MS = 10 * 60000;
...
function parsePingTarget(value: unknown): MatchmakerPingTarget | undefined {
	...
	if (labels.some((label) => label.length === 0 || label.length > 63 || !/^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/u.test(label))) return undefined;
	return { host, port };
}
...
export class MatchmakerRegionLatencyService {
	...
	public constructor(dependencies: MatchmakerLatencyDependencies, options: MatchmakerLatencyOptions = {}) {
		...
		this.targetStaleTtlMs = positiveInteger(options.targetStaleTtlMs, MATCHMAKER_LATENCY_TARGET_STALE_TTL_MS, 'targetStaleTtlMs');
		this.now = dependencies.now ?? Date.now;
	}
	public async measure(values: readonly unknown[]): Promise<MatchmakerRegionLatencies> {
		const regions = [...new Set(values.filter((value): value is string => typeof value === 'string' && KNOWN_MATCHMAKER_REGIONS.has(value)))];
	...
	private async measureMissing(regions: readonly string[]): Promise<void> {
		...
		const worker = async () => {
			while (nextIndex < pending.length) {
				...
				if (item.target) {
					...
					const candidatePorts = item.target.port === MATCHMAKER_LATENCY_FALLBACK_PORT ? [item.target.port] : [item.target.port, MATCHMAKER_LATENCY_FALLBACK_PORT];
					for (const port of candidatePorts) {
						...
						for (let attempt = 0; attempt < MATCHMAKER_LATENCY_PROBE_ATTEMPTS; attempt++) {
							try {
								const measured = await runWithTimeout(this.probeTimeoutMs, (signal) => this.dependencies.probeTarget(target, signal));
								if (typeof measured === 'number' && Number.isFinite(measured) && measured >= 0 && measured <= 60000) {
		...
		await Promise.all(Array.from({ length: Math.min(this.maxConcurrency, pending.length) }, () => worker()));
	}
