...
function defaultSocketPaths(): string[] {
	...
	const baseDirectories = [process.env.XDG_RUNTIME_DIR, process.env.TMPDIR, process.env.TMP, process.env.TEMP, '/tmp'].filter((value): value is string => Boolean(value));
	const paths: string[] = [];
...
export class DiscordRpcClient extends EventEmitter {
	...
	private consume(socket: Socket, data: Buffer): void {
		...
		while (this.receiveBuffer.length >= RPC_HEADER_BYTES) {
			...
			if (opcode === RpcOpcode.Frame && isPayloadRecord(payload) && payload.cmd === 'DISPATCH' && payload.evt === 'READY') {
				this.emit('ready');
			}
		}
	}
