...
test('ping target parsing rejects malformed envelopes and excessive target counts', () => {
	...
	assert.throws(() => parseMatchmakerPingTargets(Object.fromEntries(Array.from({ length: 33 }, (_unused, index) => [`unknown-${index}`, 'lobby.frankfurt.krunker.io:3000']))), RangeError);
});
