...
function createFakeBrowserWindow(options: FakeWebRequestOptions = {}): {
	browserWindow: Electron.BrowserWindow;
	state: FakeWebRequestState;
} {
...
function createTestPaths(t: TestContext): {
	filtersPath: string;
	root: string;
	swapDir: string;
} {
...
test('oversized or unavailable custom filters fail open without disabling built-in blockers', async (t) => {
	...
	writeTestFile(oversized.filtersPath, Array.from({ length: 4097 }, (_, index) => `*://tracker-${index}.example.com/*`).join('\n'));
	const oversizedWindow = createFakeBrowserWindow();
	const oversizedError = t.mock.method(console, 'error', (): void => {});
	const oversizedHandler = new RequestHandler(oversizedWindow.browserWindow, oversized.swapDir, false, true, true, defaultPattern, oversized.filtersPath);
	...
	const missingHandler = new RequestHandler(missingWindow.browserWindow, missing.swapDir, false, true, true, defaultPattern, missing.filtersPath);
...
test('active request features keep the BrowserFPS CORS fixer scoped to its domains', async (t) => {
	...
	assert.deepEqual(state.headersReceived?.filter.urls, ['*://browserfps.com/*', '*://*.browserfps.com/*']);
...
test('malformed mixed rules are ignored without disabling valid deduplicated blockers or swaps', async (t) => {
	...
	writeTestFile(filtersPath, ['# comments are ignored', ...malformedPatterns, '*://tracker.example.com/*', '  *://tracker.example.com/*  ', '*://ads.example.com/*'].join('\n'));
	const handler = new RequestHandler(browserWindow, swapDir, true, true, true, '*://ads.example.com/*\n*://ads.example.com/*', filtersPath);
	assert.equal(await handler.start(), true);
	assert.ok(state.beforeRequest);
	assert.equal(state.beforeRequestRegistrations, 1);
	...
	assert.equal(
		(
			handler as unknown as {
				blockingMatchers: unknown[];
			}
		).blockingMatchers.length,
		2
	);
...
test('blocker and custom matches cancel without cancelling broad-prefix misses', async (t) => {
	...
	writeTestFile(filtersPath, ['# comments are ignored', '*://tracker.example.com/*', '*://assets.krunker.io/textures/blocked.png*'].join('\n'));
	const handler = new RequestHandler(browserWindow, swapDir, true, true, true, '*://ads.example.com/*', filtersPath);
	await handler.start();
	assert.ok(state.beforeRequest);
