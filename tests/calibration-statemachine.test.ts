...
test('apply enters the provisional state with the previous selection retained for rollback', () => {
	...
	const applied = completeCalibration(state, true, 50000);
	assert.equal(applied.status, 'complete');
	assert.equal(applied.confirmation, 'pending');
	assert.equal(applied.provisionalSince, 50000);
...
test('a validated field verdict confirms the provisional profile', () => {
	const applied = completeCalibration(runToDecision(), true, 1000);
	const confirmed = confirmCalibration(applied, 2000);
	...
	assert.equal(confirmCalibration(confirmed, 3000), confirmed, 'confirming twice is a no-op');
});
test('three severe sessions roll back to the previous selection and field-reject the loser', () => {
	...
	const applied = completeCalibration(state, true, 1000);
	...
	const rolledBack = rollbackCalibration(applied, 2000);
...
test('only one automatic rollback is allowed per signature, even across resets', () => {
	const candidates = createWindowsCandidates();
	const rolledBack = rollbackCalibration(completeCalibration(runToDecision(candidates), true, 1000), 2000);
	assert.equal(canAutoRollbackCalibration(rolledBack), false);
	assert.equal(rollbackCalibration(rolledBack, 3000), rolledBack, 'a second rollback is a no-op');
	...
	const reApplied = completeCalibration(finalizeCalibration(next), true, 10000);
...
test('keep-anyway parks the provisional profile unwatched instead of reverting', () => {
	const applied = completeCalibration(runToDecision(), true, 1000);
	const keptAnyway = markCalibrationUnwatched(applied, 2000);
	...
	assert.equal(markCalibrationUnwatched(keptAnyway, 3000), keptAnyway);
});
test('a provisional profile with stalled sampling expires to unwatched after fourteen days', () => {
	const applied = completeCalibration(runToDecision(), true, 1000);
	assert.equal(calibrationProvisionalExpired(applied, 1000 + CALIBRATION_PROVISIONAL_WINDOW_MS), false);
	assert.equal(calibrationProvisionalExpired(applied, 1001 + CALIBRATION_PROVISIONAL_WINDOW_MS), true);
	assert.equal(calibrationProvisionalExpired(confirmCalibration(applied), 1001 + CALIBRATION_PROVISIONAL_WINDOW_MS), false);
});
test('declining the post-session offer persists once and survives signature resets', () => {
	...
	const declined = declineCalibrationOffer(fresh, 5000);
	assert.equal(declined.calibrationOfferDeclinedAt, 5000);
	assert.equal(declineCalibrationOffer(declined, 9000), declined, 'a second decline never overwrites the first');
	...
	assert.equal(reset.calibrationOfferDeclinedAt, 5000, 'the decline is user intent, not measurement state');
});
test('the stale-rerun prompt marker is recorded at most once', () => {
	...
	const shown = markStaleRerunPromptShown(fresh, 4000);
	assert.equal(shown.staleRerunPromptShownAt, 4000);
	assert.equal(markStaleRerunPromptShown(shown, 8000), shown);
});
test('starting a new consented run clears the previous confirmation bookkeeping', () => {
	const candidates = createWindowsCandidates();
	const applied = completeCalibration(runToDecision(candidates), true, 1000);
	const restarted = startCalibrationRun(createCalibrationState(signature, candidates, true, applied), 9999);
