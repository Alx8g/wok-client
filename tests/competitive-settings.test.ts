...
test('treats equivalent numeric and string setting values as unchanged', () => {
	assert.deepEqual(createChangedSettingsPatch({ reflection: 0 }, { reflection: '0' }), {});
});
test('does not coerce booleans into strings', () => {
	assert.deepEqual(createChangedSettingsPatch({ shadows: 'false' }, { shadows: false }), { shadows: false });
});
