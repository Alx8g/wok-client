...
const KRUNKER_CSS: KrunkerCssInventory = JSON.parse(readFileSync(join(import.meta.dirname, 'fixtures', 'krunker-css-inventory.json'), { encoding: 'utf-8' }));
function selectorsOf(css: string): string[] {
	const withoutComments = css.replaceAll(/\/\*[\s\S]*?\*\//gu, '');
	assert.ok(!/@(media|supports|container)/u.test(withoutComments), 'this parser assumes no conditional at-rules; teach it about them before adding one');
...
function hooksOf(selector: string): {
	classes: string[];
	ids: string[];
} {
	return {
...
const WOK_SELECTOR_TOKENS = ['Crankshaft', 'crankshaft', 'wok-', 'matchmaker', 'matchResultButton', 'refresh-popup', 'customKeybindSetting'];
...
function isLiveKrunkerSelector(selector: string): boolean {
	...
	return hooks.classes.every((name) => KRUNKER_CSS.classes.includes(name)) && hooks.ids.every((name) => KRUNKER_CSS.ids.includes(name));
}
...
const REMOVAL_DECLARATIONS = [/display\s*:\s*none/u, /visibility\s*:\s*hidden/u, /opacity\s*:\s*0(?![.\d])/u, /content\s*:\s*none/u];
const BLUR_ALLOWED_SELECTORS = new Set(['#menuWindow', '#popupContent', '.defaultModal', '.socialModal', '#itemViewPop', '#itemPurcPop', '.io-popup-container', '#matchmakerPopupContainer']);
...
function parseColour(value: string): Rgba | undefined {
	...
	if (hex) {
		...
		return [Number.parseInt(digits.slice(0, 2), 16), Number.parseInt(digits.slice(2, 4), 16), Number.parseInt(digits.slice(4, 6), 16), 1];
	}
...
function over(top: Rgba, bottom: Rgba): Rgba {
	return [top[0] * top[3] + bottom[0] * (1 - top[3]), top[1] * top[3] + bottom[1] * (1 - top[3]), top[2] * top[3] + bottom[2] * (1 - top[3]), 1];
}
...
test('every bundled theme ships the palette and shared layers it resolves to', () => {
	...
	for (const theme of BUNDLED_THEMES) {
		...
		assert.deepEqual(source.kind === 'bundled' ? source.assets : [], [...THEME_LAYER_ASSETS, themeAssetName(theme.id)]);
...
test('every bundled palette defines the whole documented variable contract', () => {
	for (const theme of BUNDLED_THEMES) {
		...
		for (const variable of THEME_VARIABLES) {
			assert.match(palette, new RegExp(`${variable.name}\\s*:\\s*\\S`, 'u'), `assets/${themeAssetName(theme.id)} does not set ${variable.name}`);
		}
	}
});
...
test('every documented variable is actually consumed by a shared layer', () => {
	...
	for (const variable of THEME_VARIABLES) {
		...
		assert.ok(layers.includes(`var(${variable.name}`), `${variable.name} is documented but nothing reads it`);
	}
});
test('every shared-layer read falls back to a stock value when a palette omits it', () => {
	for (const asset of THEME_LAYER_ASSETS) {
		...
		for (const read of reads) {
			assert.equal(read[1], ',', `assets/${asset} reads a variable with no fallback, so a partial palette would break it: ${read[0]}`);
		}
	}
});
test('the base layer stays inside surfaces WOK adds to the page', () => {
	for (const selector of selectorsOf(readAsset(THEME_BASE_ASSET))) {
		assert.ok(isWokOwnedSelector(selector), `assets/${THEME_BASE_ASSET} styles '${selector}', which is not a WOK surface`);
	}
});
test('every hook the game layer targets exists in the committed Krunker stylesheet inventory', () => {
	...
	for (const selector of selectors) {
		...
		for (const name of hooks.classes) {
			assert.ok(
				KRUNKER_CSS.classes.includes(name),
...
test('the game layer remaps only Krunker custom properties that Krunker still defines', () => {
	const remapped = [...readAsset(THEME_GAME_ASSET).matchAll(/^\t(--[a-z][a-z0-9-]*)\s*:/gmu)].map((match) => match[1]).filter((name) => !name.startsWith('--wok-'));
	assert.ok(remapped.length > 0);
	for (const name of new Set(remapped)) {
		assert.ok(KRUNKER_CSS.customProperties.includes(name), `assets/${THEME_GAME_ASSET} remaps ${name}, which Krunker's stylesheets no longer define`);
	}
});
test("the game layer leaves WOK's own surfaces to the base layer", () => {
	for (const selector of selectorsOf(readAsset(THEME_GAME_ASSET))) {
		assert.ok(!/[Cc]rankshaft|wok-mark/u.test(selector), `assets/${THEME_GAME_ASSET} styles '${selector}'; WOK's own surfaces belong in ${THEME_BASE_ASSET}`);
	}
});
test('palettes stay inside WOK surfaces and hooks Krunker really declares', () => {
	for (const asset of PALETTE_ASSETS) {
		for (const selector of selectorsOf(readAsset(asset))) {
			assert.ok(isWokOwnedSelector(selector) || isLiveKrunkerSelector(selector), `assets/${asset} styles '${selector}', which is neither a WOK surface nor a live Krunker hook`);
		}
	}
});
	...
	for (const asset of ALL_THEME_ASSETS) {
		...
		for (const rule of rulesOf(css)) {
			...
			for (const name of [...hooks.classes, ...hooks.ids]) {
				assert.ok(!OFF_LIMITS_HOOKS.includes(name), `assets/${asset} styles '${name}' in '${rule.selector}'; that hook carries information, not decoration`);
			}
			for (const pattern of REMOVAL_DECLARATIONS) {
				assert.ok(!pattern.test(rule.body), `assets/${asset} removes something in '${rule.selector}'; a theme restyles, it never hides`);
			}
		}
		assert.ok(!/\banimation\s*:/u.test(css.replaceAll(/\/\*[\s\S]*?\*\//gu, '')), `assets/${asset} declares an animation; themes are static so they cannot cost frames`);
	}
});
test('backdrop-filter is confined to surfaces that only exist in menus', () => {
	...
	for (const asset of ALL_THEME_ASSETS) {
		for (const rule of rulesOf(readAsset(asset))) {
			...
			blurred++;
			assert.ok(
				BLUR_ALLOWED_SELECTORS.has(rule.selector),
				`assets/${asset} blurs '${rule.selector}', which is not on the menu-only allowlist. ` + 'A backdrop-filter that can be on screen during gameplay costs the player frames.'
			);
		}
	}
...
test('every palette keeps its text readable on the surface it is drawn on', () => {
	for (const theme of BUNDLED_THEMES) {
		...
		for (const [label, foreground, background, minimum] of pairs) {
			...
			assert.ok(ratio >= minimum, `${theme.id}: ${label} is ${ratio.toFixed(2)}:1, below the ${minimum}:1 this pair needs`);
		}
	}
});
...
test('resolveTheme refuses anything that could escape the css folder', () => {
	for (const selection of ['../outside.css', '..\\outside.css', '/etc/passwd.css', 'C:\\windows\\evil.css', 'nested/dir/theme.css', `${'a'.repeat(200)}.css`]) {
...
test('the dropdown drops duplicates and anything that is not a plain css filename', () => {
	...
	assert.deepEqual(
		options.values.filter((value) => value.endsWith('.css')),
		['mine.css']
	);
