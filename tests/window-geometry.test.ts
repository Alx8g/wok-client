...
test('maximized on a chosen display starts from that display, so the native maximize expands onto it', () => {
	assert.deepEqual(
		resolveGameplayWindowGeometry('maximized', secondDisplay, WINDOW_SCALE, true),
		resolveGameplayWindowGeometry('windowed', secondDisplay, WINDOW_SCALE, true)
	);
});

test('borderless already carries display coordinates, so explicit placement changes nothing', () => {
	assert.deepEqual(
		resolveGameplayWindowGeometry('borderless', secondDisplay, WINDOW_SCALE, true),
		resolveGameplayWindowGeometry('borderless', secondDisplay, WINDOW_SCALE)
	);
});
